...
namespace ERP.Repository.Repositories
{
    public class TransactionRepository : GenericRepository<transaction>, ITransactionRepository
    {
        ...
        public transactionviewmodel GetById(int tra_id)
        {
            ...
            var list_se_type = _dbContext.service_type.ToList();
            if (tran != null)
            {
            ...
            foreach (var cuo in list_customer_order_service)
            {
                ...
                foreach (order_service ord in list_order_service)
                {
                    ...
                    if (se != null)
                    {
                        ...
                        {
                            if (add.se_type == j.styp_id)
                            {
                                add.se_type_name = j.styp_name;
                            }
                        }
        ...
        public PagedResults<transactionviewmodel> GetAllPageSearch(int pageNumber, int pageSize, DateTime? start_date, DateTime? end_date, string search_name, int company_id, int curr_id, int? tra_rate)
        {
            ...
            foreach (var i in results)
            {
                ...
                //Bat theo Enums
                var tra_type= _dbContext.transaction_deal_type.Where(x => x.trand_id == i.tra.tra_type).FirstOrDefault();
                if (tra_type != null) transactionview.tra_type_name = tra_type.trand_name;
